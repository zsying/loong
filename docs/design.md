...
- **`web`（`components/web`）**：Config 只含 `listen` / `tls` / `log`。**不 import user/auth、不订阅业务事件**；默认中间件 Recover（panic → 500）+ 请求日志（Debug）。RequestLog 注册在前、Recover 在内，所以日志能记到 Recover 转换后的最终状态码。
